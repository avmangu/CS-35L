...
void *colorEncode(void * threader) {
  Vec3 camera_pos;
  ...
  /* for every pixel */
  for(int px=split; px<width; px+=nthreads)
    {
...
int main( int argc, char **argv ) {
  ...
  for (int i = 0; i < nthreads; i++) {
    ids[i] = i;
    complete = pthread_create(&threads[i], NULL,
			      colorEncode, (void *)(i + ids));
  ...
  for (int i = 0; i < nthreads; i++) {
    complete = pthread_join(threads[i], NULL);
